					...
					if(r){
						if(jQuery("#hidItemIds").val()==''){
							...
							jAlert('Items are added to menu group successfully!', 'Alert', function () {										
								window.location.href = "setup_rest_menu_details_page.php?menu="+ jQuery("#hidMenu").val()+"&menu_id="+jQuery("#hidMenuGroupID").val();
							});
							return false;
						}
						jQuery.ajax({
    ...
    <div class="rightpanel">
        ...
        <ul class="breadcrumbs">
            ...
            <li>Setup <span class="separator"></span> Restaurant <span class="separator"></span></li>
                ...
                <li><!--<a href="menu_details_page.php">-->Menu Details<!--</a>--></li>
...
if(mysql_num_rows($query)!=0)
{
	...
	while ($row_allworker = mysql_fetch_array($query)) 
	{
            ...
            <div class="pagetitle">
                <h5>Add and Edit Menu Details</h5>
                <h1>Menu Details</h1>
            </div>
        </div><!--pageheader-->
        ...
        <div class="maincontent">
            <div class="maincontentinner">
                <div class="widgetbox" style="width:30%;float:left;">
                            <div class="headtitle">
                                <h4 class="widgettitle">Menu Details</h4>
                            </div>
			    <div class="widgetcontent" style="min-height:255px;">
                                ...
                                <table id="dyntable2" class="table table-bordered responsive">
                                    ...
                                    <tbody>
                                <?php 
                                    $sql = "SELECT lart.*,per.printer_name,lmam.id as modifier
                              FROM location_menu_articles as lart
                              LEFT JOIN location_printers as per on  lart.printer_id=per.id
                              LEFT JOIN location_menu_article_modifiers lmam on lmam.item_id=lart.id 
							  WHERE lart.Status ='Active' AND lart.item<>'' AND lart.location_id={$_SESSION["loc"]}
                               GROUP BY lart.id ORDER BY priority asc ";
                                    $query = mysql_query($sql) or die(mysql_error());
                        while ($lnrv = mysql_fetch_assoc($query)) {
                            ?>
                                        <tr id='<?php echo $lnrv["id"];?>' class="menuitems"> 
                                           <td >
                                               <div data-id='<?php echo $lnrv["id"];?>' data-itemname="<?php echo $lnrv['item']; ?>" data-plu="<?php echo $lnrv['plu']; ?>" data-drink="<?php echo ucfirst($lnrv['drink']); ?>" data-fire="<?php echo $lnrv['fire_order']; ?>" data-taxable="<?php echo ucfirst($lnrv['taxable']); ?>" data-printer="<?php echo $lnrv['printer_name']; ?>" data-price="<?php echo $currency.number_format($lnrv['price'], 2); ?>" class="draggable menuitem">
                                                <ul style="list-style:none;">
                                                <li>
                                                <input type="checkbox" class="checkbox menuitems_checkBox"  rel="<?php echo $lnrv["id"];?>"   style="margin-right:5px; margin-top:0px;" ><?php echo $lnrv['item']; ?></li>
...
$qrgrv = mysql_query($query) or die(mysql_error());
			   ?>
                <div class="row-fluid">
  ...
  <tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    <td align="left" valign="top" width="95%" style="color:#FFFFFF; padding:0px 10px 0px 10px " >Groups / Items</td>
	 <td align="center" valign="top" width="5%" >
     <a data-toggle="modal" data-target="#edit_modifier_modal" href="ajax-edit-menu-default.php?newgroup_id=<?=$_REQUEST['newgroup_id'];?>&id=<?php echo $lngrv["id"];?>&menu_id=<?php echo $menu; ?>" style="background:none;!important; ">
   <span class="icon-plus addmenu"></span></a></td>
	</tr>
	</table>
                        ...
                        <div  id="accordion">
                           ...
                           <?php while($lngrv = mysql_fetch_assoc($qrgrv)){ 
						   ...
						   <div  class="s_panel droppable" id="ac_menugroup_<?php echo $lngrv["id"];?>" data-id="<?php echo $lngrv["id"];?>" data-menu="<?php echo $menu;?>" data-target="<?php echo $lngrv["id"];?>">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0" class="accordion-toggle" >
                                                            <tr id="del_<?php echo $lngrv["id"];?>">
                                                              <td align="left" valign="top" width="95%" style="padding: 4px 0 0 4px !important;"><span style="margin-right:5px;float:left;"><?php if($lngrv['image']!=""){?><img src="<?php echo APIIMAGE.'images/'.$lngrv['image'];?>" onerror="this.src='images/group-noimage.png'" width="32"><?php }else{?> <img src="images/group-noimage.png" width="32" > <?php } ?></span><span style="height:auto; vertical-align:middle;float:left;margin-top:7px"><?=$lngrv['menu_group']?></span></td>
                                                             <td align="left" valign="top" width="5%" style="vertical-align:top;">
                                                             
                                                             <a data-toggle="modal" data-target="#edit_modifier_modal" href="ajax-edit-menu-default.php?id=<?php echo $lngrv["id"];?>&menu_id=<?php echo $menu; ?>" style="background:none !important;  line-height: 10px !important; margin-top: 11px !important; float:left; padding: 1px 2px  !important; "><span class="a"><img src="images/edit.png"></span></a>
                                                             
                                                             
                                                             <a href="javascript:void(0)" style="background:none !important;  line-height: 10px !important; margin-top: 11px !important; width: 16px; float:left; padding: 1px 2px !important; "><img onClick="DeleteMenuGroup('<?php echo $lngrv["id"];?>')" src="images/Delete.png"></a>
                                                             </td>
                                                            </tr>
                                                          </table>
							  ...
							  <?php
                                                                ...
                                                                $qritrv = mysql_query($query2) or die(mysql_error());
                                                                if (mysql_num_rows($qritrv) != 0)
																{
																
																?>
																	
                            <div>
                               <table class="table table-bordered responsive" rel = "<? echo $lngrv['id'];?>" id="menu_table_<? echo $lngrv['id'];?>">
                    <colgroup>
                        ...
                        <col class="con0" style="width:12% " />
						 ...
						 <col class="con1" style="width:35px;"/>
                    </colgroup>
                    <thead>
                        <tr>
                            <th class="head0">Priority</th>
                            <th class="head1">Menu Item</th>
							 <th class="head0">PLU</th>
							  <th class="head1">Drink</th>
							   <th class="head0">Fire</th>
                               <th class="head1">Taxable</th>
							   <th class="head0">Printer</th>
							    <th class="head1">Price</th>
								<th class="head1 nosort" valign="middle" style="text-align:center;width:35px;" >Action</th>
                        </tr>
                    </thead>
                    <tbody>
					  <?php while($lnitrv = mysql_fetch_assoc($qritrv)){
                      		$deletable = TRUE;
							if( mysql_num_rows(mysql_query("SELECT id FROM client_order_items WHERE menu_group = '".$lnitrv['menu_id']."' AND  menu_item_id = ".$lnitrv['item_id'])) > 0 ||  mysql_num_rows(mysql_query("SELECT id FROM client_order_items_modifier WHERE menu_item_id = ".$lnitrv['item_id'])) > 0) { 
								$deletable = FALSE;
							} ?>
							<tr class="gradeX delete_items_<?php echo $lnitrv['id']?>"  id="delete_items_<?=$lnitrv['id']?>">
                            <td  data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
                                                           data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
                                                           class="codedata" ><a style="color:#666666;display:block;"><?=$lnitrv['priority']?></a></td>
                           
                            <td data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
						  								   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
						   								   class="codedata" ><a style="color:#666666;display:block;"><?=$lnitrv['item']?></a></td>
                                                           
							<td data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
						  								   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
						   								   class="codedata" ><a  style="color:#666666;display:block;"><?=$lnitrv['plu']?></a></td>
                                                           
							<td  data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
						  								   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
						   								   class="codedata" ><a style="color:#666666;display:block;"><?php echo ucfirst($lnitrv['drink']); ?></a></td>
                                                           
							<td data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
    	                                                   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
	                                                       class="codedata" ><a style="color:#666666;display:block;"><?php echo $lnitrv['fire_order']; ?></a></td>
                                                           
                            <td data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
								  						   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
                                                                                                                   class="codedata"  ><a style="color:#666666;display:block;"><?php echo ucfirst($lnitrv['taxable']); ?></a></td>
                                                           
                           <td data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
						  								   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
						   								   class="codedata" ><a  style="color:#666666;display:block;"><?php echo $lnitrv['printer_name']; ?></a></td>
                                                           
					       <td data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
						  								   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
						   								   class="codedata" style="text-align:right;" ><a style="color:#666666;display:block;"><?= $currency.number_format($lnitrv['price'], 2)?></a></td>
                                                           
						 <td valign="middle" style="text-align:center;width:35px;" >
                                                     <a data-toggle="modal" data-target="#edit_item_modal" href="ajax-edit-menu-item.php?itemid=<?php echo $lnitrv["item_id"];?>&menu=<? echo $menu;?>&group=<? echo $lngrv["id"];?>" style="background:none;!important;  line-height: 10px !important;
    margin-top: 5px !important;
    padding: 1px !important; "><span class="a"><img src="images/edit.png"></span></a>
								 						<!--   <a data-id="<? echo $lnitrv['id'];?>" data-menu_id="<? echo $lnitrv['menu_id'];?>"
														   data-item_id="<? echo $lnitrv['item_id'];?>" 
														   data-item="<? echo $lnitrv['item'];?>" 
						  								   data-price="<? echo $lnitrv['price'];?>" data-priority="<? echo $lnitrv['priority'];?>" 
						   								   class="codedata" style="cursor:pointer;"><img src="images/edit.png" style="margin-right:8px; "></a>-->
                                                            
								 <span <?php if ($deletable) { echo 'class="deletemenu" data-id="'.$lnitrv['id'].'"'; ?> 
                                 onClick='itemtodelete("<?php echo $lnitrv['id']?>");' <?php }else {?>                                  
                                 onClick='itemtoInactive("<?php echo $lnitrv['id']?>");' <?php }?> 
                                 style="cursor:pointer!important;" ><img src="images/Delete.png"></span></td>
                        </tr>
						<?
							$lastPriority = $lnitrv['priority']+1;
						}
						?>
...
var table<? echo $lngrv['id'];?> = document.getElementById('menu_table_<? echo $lngrv['id'];?>');
